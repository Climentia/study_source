...
def parameter_fanc():
    ...
    df_change_weather = pd.read_csv('H:/study/source/weather/change_weather_15.csv',
                                    encoding='cp932')
    ...
    for parameter in parameter_list:
        ...
        for i in range(df_change_weather['year'].count()-1):
            ...
            df = pd.concat([df, df2])
            count = count + 1
        write_path = './change_error/change_error_' + 'lambda_' + str(lambda_num) + '_iterate_' + str(iterate) + '_p' + str(pyramid) + '_15.csv'
...
def persist_fanc():
    ...
    for i in range(1):
        ...
        for i in range(df_change_weather['year'].count()-1):
            ...
            df = pd.concat([df, df2])
            print(df)
        write_path = './change_error/change_error_' + 'persist_nv_15.csv'
        df.to_csv(write_path, index=False)
