        ...
        for j in range(day_max):
            ...
            for tt in range(26):
                try:
                    ...
                    column_list_0001 = ['env', 'nv', 'nv_meshint', 'preal', 'preal_meshint', 'ppred', 'nv_error', 'nv_perror', 'nv_max_perror', 'nv_meshint_error', 'nv_meshint_perror', 'nv_meshint_max_perror']
                    column_list_001 = ['id_lat_mesh', 'id_lng_mesh', 'pvrate']
                    column_list_01 = ['observed_max']
                    for my_col in df.columns:
                        ...
                        elif my_col in column_list_01:
                            df2[my_col] = df2[my_col].map(lambda x: float(Decimal(str(x)).quantize(Decimal('0.1'), rounding=ROUND_HALF_UP)))
                    # print(df2)
                    # input()
