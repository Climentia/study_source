...
def pdcal(path, df_change, df_unchange):
    ...
    df_id_annual = df_id_annual[(df_id_annual['time'] > 7.5) & (df_id_annual['time'] < 16.5)]
    df_id_annual['e'] = abs((df_id_annual['preal'] - df_id_annual['ppred'])*100 / df_id_annual['observed_max'])
    rmse_all = (df_id_annual['e'].mean())
    ...
    rmse_change = (df_id_annual_change['e'].mean())
    ...
    rmse_unchange = (df_id_annual_unchange['e'].mean())
...
df_write.to_csv('id_mape.csv', index=False, encoding ='cp932')
