...
df_400 = pd.read_csv('F:/study/id_data/extract/extract_15.csv', encoding='cp932')
df_40 = df_400[['id', 'id_prefecture']]
write_path = './mape_' + str(place) + '_persist_nv.csv'
count = 0
    ...
    for j in range(day_max):
        ...
        for tt in range(24):
            try:
                ...
                read_path = 'F:/study/error_data/parameter_persist_nv_30min/' + str(month) + '月/' + str(month) + '月' + str(day) + '日/' + 'error_' + str(year) + str(month) + str(day) + str(tt2) + '_' + str(mesh_range) + '_presist_nv_30min.csv'
