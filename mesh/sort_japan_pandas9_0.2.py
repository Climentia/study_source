...
def sort(YY, MM, DD, TT):
    ...
    print(path1)
    path3_1 = "G:\\id_time\\mesh_place0.2\\" + MM + "月"
    path3_1_ch = os.path.exists(path3_1)
    if path3_1_ch == False:
        os.mkdir(path3_1)
    path3_2 = "G:\\id_time\\mesh_place0.2\\" + MM + "月\\" + MM + "月" + DD + "日"
    path3_2_ch = os.path.exists(path3_2)
    if path3_2_ch == False:
        os.mkdir(path3_2)
    # mesh化書き込みファイル
    path3 = "G:\\id_time\\mesh_place0.2\\" + MM + "月\\" + MM + "月" + DD + "日" + "\\NV" + YY + MM + DD + TT + "_time_sort.csv"
    ...
    if check == 0:
        ...
        with open(path3, "w") as fw:
            for i in range(43):
                for j in range(60):
                    lat = round(31.2 + i * 0.2, 2)
                    lng = round(129.6 + j * 0.2, 2)
        ...
        with open(path3, "w") as fw:
            ...
            for i in range(43):
                for j in range(60):
                    lat = round(31.2 + i * 0.2, 2)
                    lng = round(129.6 + j * 0.2, 2)
                    mesh_place = "G:/id_time/data/mesh_data_0.2/id_list_" + str(lat) + "_" + str(lng) + ".txt"
