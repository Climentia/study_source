...
for i in range(12):
    month = i + 1
    path_month = 'F:/study/error_data/parameter_persist_1hour/' + str(month) + '月/'
    ...
    for j in range(day_max):
        ...
        for tt in range(24):
            try:
                ...
                read_path_old = 'F:/study/preprocessing_data/mesh_com/' + str(month) + '月/' + str(month) + '月' + str(day) + '日/mesh_com' + str(year) + str(month) + str(day) + str(round(tt2 - 1.0, 1)) + '.csv'
                write_path = path_day + 'error_' + str(year) + str(month) + str(day) + str(tt2) + '_' + str(mesh_range) + '_persist_1hour.csv'
